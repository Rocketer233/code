...
class LlamaGQttention(nn.Module):
    ...
    def __init__(self, config: LlamaConfig, layer_id: int = 0):
        ...
        self.k_groups = config.groups_idx["k"][layer_id] # groups_idx has to have the shape: [num_layers, {num_groups, len_grop} <- not fixed]
        self.v_groups = config.groups_idx["v"][layer_id]
        ...
        self.flops = 0
        self.num_params = 2 * self.hidden_size ** 2 + 2 * self.hidden_size * self.num_groups * self.head_dim

    def _shape(self, tensor: torch.Tensor, seq_len: int, bsz: int, num:int):
        return (
    ...
    def forward(
        ...
        kv_flops = bsz * q_len * self.hidden_size * self.num_groups * self.head_dim
        qo_flops = bsz * q_len * self.hidden_size ** 2
        ...
        self.flops += 2 * kv_flops + qo_flops + bsz * q_len * self.hidden_size
        
        key_states = (
            self.k_proj(hidden_states)
            .view(bsz, q_len, self.num_groups, self.head_dim) # only calculates a single head
        )
        key_states = group_expand(key_states, self.k_groups, self.num_groups, query_states.size())
        value_states = (
            self.v_proj(hidden_states)
            .view(bsz, q_len, self.num_groups, self.head_dim) # only calculates a single head
        )
        value_states = group_expand(value_states, self.v_groups, self.num_groups, query_states.size())
        ...
        self.flops += bsz * self.num_groups * q_len * self.head_dim * kv_seq_len
        ...
        self.flops += bsz * q_len ** 2 * self.head_dim
...
class LlamaModel(LlamaPreTrainedModel):
    ...
    def __init__(self, config: LlamaConfig):
        ...
        if len(config.groups_idx["k"]) != config.num_hidden_layers:
            raise ValueError(
